...
<!-- NOTE: I should create a new file for all of the below notes -->
...
1. Settings tab: Rules > Rulesets > Click the New ruleset dropdown and select New branch ruleset

- the Ruleset Name as Protect main and change the Enforcement status to Active
- Find the Targets section and use the Add target dropdown to add 2 entries
  - Add the Include default branch option to ensure protections aren't bypassed by switching the default branch
  - Use the include by pattern option and enter the pattern main
...
- Find Dependabot version updates and click the Enable button - This will open an editor to create a configuration file
  ...
  - At the top, change the branch to prepare-to-collaborate (our ruleset won't let us directly change files on main)
  - Set the package-ecosytem to pip so Dependabot will automatically monitor our Python requirements
